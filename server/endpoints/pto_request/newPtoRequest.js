...
function isValidPto(e_id, title, desc, start_date, end_date, date_due, assigned_to) {
  let valid = false
  if (e_id !== null && title.length > 0 && desc.length > 0 && start_date !== null && end_date !== null && date_due !== null && assigned_to.length > 0) {  //first checks that no empty values were given
...
function newPtoRequest(app){
  app.post('/api/empTasks/newPtoRequest', checkLoggedIn, async(req,res) => {
    ...
    if (isValidPto(e_id, req.body.title, req.body.description, req.body.start_date, req.body.end_date, req.body.date_due, req.body.assigned_to)) {  //check the validity of the data given
      const arr = []
      for (let i = 0; i < req.body.assigned_to.length; i++) { //loop through the emails given in assigned_to
        console.log(req.body.assigned_to[i])
        try { //try to find if the employee with the email can be found
      ...
      res.send(arr)
    } else {  //if data is not valid, sends error to the user
      res.status(400)
